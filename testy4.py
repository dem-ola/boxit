...
def yell(func):
    ...
    def wrap(*args, **kwargs):
        try:
            ...
            if kwargs.get('expect_fail', False):
                print('...')
            else:
...
def test_naming(i, V, action, key, item, name1, expect_fail=False):
    ...
    if action == 'get':
        name = item.name if issubclass(item.__class__, Vessel) \
                else V.getname(item)
        assert name == name1
...
for i in range(len(vessels)-1):
    ...
    _ds.p()
    _ds.p()
    _de.p(V.__str__().upper())

    # names for vessel items
    if v == Box:        names = list(map(lambda x: 'b'+x, names))
    elif v == Crate:    names = list(map(lambda x: 'c'+x, names))
    ...
    # but if item already named and another given - new name is used - see put
    suite = (
